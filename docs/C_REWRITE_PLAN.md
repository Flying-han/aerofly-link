...
- 版本: 1.0（2026-09-18）
- 状态: P0 进行中（协议层骨架已在 `client-c/` 落地并通过单元测试）
...
| 阶段 | 内容 | 验收标准 | 状态 |
| --- | --- | --- | --- |
| **P0 协议层骨架** | core 三模块 + C 单元测试 + build.cmd | 测试全绿；与 pytest 用例数值一致 | ✅ 本 PR |
| **P1 传输与桥接** | `transport`（连接/退避/会话状态机）+ `bridge`（12345/12346 客户端）+ Mock 进程互验 | 无头模式登录 Mock+本地 FSD；断线重连符合退避预算 | 待开发 |
| **P2 无头客户端** | `app` 状态机 + CLI（配置文件/控制台日志/优雅退出）；与 Python 版并跑对比 24h | G2 互操作通过；内存/CPU 预算实测达标；**行为基准自此切换为 C** | 待开发 |
| **P3 UI** | 技术选型 ADR → 实现（连接/应答机/飞行计划/日志四面板等价功能） | 冒烟截图对照 Python 版；无功能回退 | 待开发 |
